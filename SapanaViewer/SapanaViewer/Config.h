//
//  Config.h
//  SapanaViewer
//
//  Created by Jan Meier on 06.03.14.
//  Copyright (c) 2014 Visualization and MultiMedia Lab, University of Zurich, Switzerland. All rights reserved.
//

#import <Foundation/Foundation.h>

extern const float CORNER_RADIUS_TOPIC_SUBVIEW;
extern const float BORDER_WIDTH_TOPIC_SUBVIEW;
extern const float CORNER_RADIUS_TOPIC_SUBSUBVIEW;
extern const float SV_UPDATE_INTERVAL;
extern const float NAVIGATOIN_SPEED_REDUCTION_FACTOR;
extern const UIFont * TABLE_FONT;


//http://stackoverflow.com/questions/1710180/change-system-font-for-iphone-application-entire-application
#define TABLE_FONT [UIFont fontWithName:@"HelveticaNeue-Light" size:16]
#define TABLE_HEADING_FONT [UIFont fontWithName:@"HelveticaNeue-Light" size:20]