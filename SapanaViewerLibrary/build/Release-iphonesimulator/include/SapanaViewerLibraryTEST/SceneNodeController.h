...
public:
    ...
    virtual std::vector<  MatrixForGUI >  getTransformationMatrixList() = 0;
 ...
 */
    class SceneNodeController : public spv::IController
{
public:
     ...
     * updated.
     */
    virtual void update() const {impl_->update();}
    
    virtual std::vector< std::shared_ptr< const spv::IController > > getChildController() const {return impl_->getChildController();}
...
#pragma mark - Transformatin List Manipulation
     ...
     */
    std::vector< MatrixForGUI >  getTransformationMatrixList() {return impl_->getTransformationMatrixList();}
